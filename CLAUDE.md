...
- **Robots & agents**: robots.txt explicitly allows AI crawlers (GPTBot, ClaudeBot, PerplexityBot, Google-Extended, CCBot, …) — the owner WANTS agents reading the site; `/public/llms.txt` is the LLM-friendly summary (facts + links, keep in sync with big fact changes). Static `public/og.png` (1200x630, regenerate from `web/scripts/og.svg` via rsvg-convert) is the default OG/twitter image. `/privacy` has a consent-withdraw button (`consent-withdraw.tsx`).
